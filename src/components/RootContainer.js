...
function RootContainer() {
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:8080/breaches?email=${email}`).then(async (resp) => {
      if (resp.status !== 200) {
        resp.json().then(respJson => setErrorMsg(respJson.friendly_error || 'An unknown error has occurred. Please try again.'))
        return
      }
  ...
  return (
    <div>
      <ErrorDisplay open={errorMsg.length > 0} onClose={() => setErrorMsg('')} msg={errorMsg} />
